...
const GOOGLE_PAGESPEED_API_KEY = process.env.GOOGLE_PAGESPEED_INSIGHTS_API_KEY || 
                                   process.env.GOOGLE_PAGESPEED_API_KEY || 
                                   process.env.GOOGLE_PAGESPEED_INSIGHTS_KEY;
...
async function updatePageSpeedData() {
  ...
  try {
    // Find the business profile - use the specific profile ID from UI
    const PROFILE_ID = 'cmhxul8du0005rq8xos8x0qjq';
    ...
    console.log(`   Using API Key: ${GOOGLE_PAGESPEED_API_KEY?.substring(0, 10) || 'N/A'}...`);
